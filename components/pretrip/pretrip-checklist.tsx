...
export function PretripChecklist({ tripId, initialItems }: { tripId: string; initialItems: PretripItem[] }) {
  ...
  async function toggle(key: PretripKey) {
    ...
    try {
      const response = await fetch(`/api/trips/${tripId}/checklist`, {
        ...
        body: JSON.stringify({ item_key: key, completed: next, notes: current?.notes ?? null }),
      });
  ...
  return (
    <section className="pretrip-section" aria-label="Pré-viagem">
      <div className="pretrip-heading">
        <span className="auth-eyebrow">Pré-viagem</span>
        <h2>Prepare o essencial antes de embarcar</h2>
        <p>Marque o que já resolveu. Requisitos oficiais de entrada dependem do seu passaporte, residência, destino e datas e devem ser confirmados em fontes oficiais.</p>
      ...
      <div className="pretrip-grid">
        {PRETRIP_CATALOG.map((entry) => {
          const done = Boolean(map.get(entry.key)?.completed);
          return (
