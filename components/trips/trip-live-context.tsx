...
type ForecastResponse = { daily?: Daily };
...
export function TripLiveContext({ city, countryCode, startDate, endDate }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const geoResponse = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(city)}&countryCode=${encodeURIComponent(countryCode)}&count=1&language=pt&format=json`);
        const geo = await geoResponse.json() as GeocodingResponse;
        const found = geo.results?.[0];
        if (!found || !active) return;
        setPlace(found);
        const forecastResponse = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${found.latitude}&longitude=${found.longitude}&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max&timezone=auto&forecast_days=16`);
  ...
  return <section className="trip-live-context" aria-label="Contexto ao vivo do destino"><div className="trip-live-heading"><div><span className="detail-label">Destino ao vivo</span><h2>Clima e localização</h2><p>Informações atuais para ajudar a ajustar o roteiro antes de sair.</p></div><span className="live-badge">● AO VIVO</span></div><div className="trip-live-grid"><article className="trip-weather-card"><div className="live-card-title"><span>☀</span><div><strong>Previsão do tempo</strong><small>{city}</small></div></div>{loading ? <p className="live-muted">Buscando previsão atualizada…</p> : tripForecast.length ? <div className="weather-days">{tripForecast.map(day => <div className="weather-day" key={day.date}><small>{new Intl.DateTimeFormat("pt-BR", { weekday: "short", day: "2-digit" }).format(new Date(`${day.date}T12:00:00`))}</small><b>{weatherIcon(day.code)}</b><strong>{Math.round(day.max)}°</strong><span>{Math.round(day.min)}°</span><em>☂ {day.rain ?? 0}%</em><i>{weatherLabel(day.code)}</i></div>)}</div> : <div className="forecast-future"><strong>Previsão ainda não disponível para as datas da viagem.</strong><p>A previsão detalhada aparece automaticamente quando a viagem entrar na janela de até 16 dias.</p></div>}<small className="weather-source">Dados meteorológicos: Open-Meteo</small></article><article className="trip-map-card"><div className="live-card-title"><span>⌖</span><div><strong>Mapa do destino</strong><small>{place ? `${place.name}${place.country ? `, ${place.country}` : ""}` : city}</small></div></div>{mapUrl ? <iframe title={`Mapa de ${city}`} src={mapUrl} loading="lazy" referrerPolicy="no-referrer-when-downgrade" /> : <div className="map-loading">{loading ? "Localizando destino…" : "Mapa indisponível no momento."}</div>}<a href={mapsLink} target="_blank" rel="noreferrer">Abrir mapa completo ↗</a></article></div></section>;
}
